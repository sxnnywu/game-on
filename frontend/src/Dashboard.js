...
const [showLeagueDetails, setShowLeagueDetails] = useState(false);
  ...
  const generateLeagueCode = () => {
    ...
    const link = `https://gameon.com/join/${code}`;
    setLeagueCode(code);
    setInviteLink(link);
    return { code, link };
  };
  ...
  if (joinCode.trim()) {
    ...
    // TODO: Add logic to actually join the league in MongoDB
  }
};
...
const leaveLeague = (leagueId) => {
  ...
  // TODO: Add logic to leave league in MongoDB
};
...
{showLeagueDetails && selectedLeague && (
  <div className="fixed inset-0 z-10 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
    <div className="bg-white/10 backdrop-blur-lg rounded-3xl border border-white/20 max-w-4xl w-full max-h-[90vh] overflow-y-auto animate-fade-in">
      {/* Header */}
      <div className="p-8 border-b border-white/10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          {/* Add other info like season, scoring, playoff */}
        </div>
        {/* League Code */}
        ...
        <div className="space-y-3">
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 border border-white/20 animate-slide-up">
              ...
              <div className="space-y-4">
                {mockLeagues.map((league, index) => (
                  <div 
                    key={league.id} 
                    className="bg-white/10 rounded-2xl p-4 border border-white/10 hover:bg-white/20 transition-all duration-300 transform hover:scale-105"
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 border border-white/20 animate-slide-up">
              ...
              <div className="space-y-4">
                {mockMatchups.map((match, index) => (
                  <div 
                    key={match.id} 
                    className="bg-white/10 rounded-2xl p-4 border border-white/10 hover:bg-white/20 transition-all duration-300"
                    style={{ animationDelay: `${index * 0.1}s` }}
                  >
                    <div className="flex items-center justify-between mb-2">
                        ...
                        {match.status === 'upcoming' ? (
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span className={`text-sm font-semibold ml-2 ${
                              match.status === 'won' ? 'text-green-400' : 'text-red-400'
                            }`}>
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8 border border-white/20 animate-slide-up">
              ...
              <div className="space-y-3">
                {['Sarah M.', 'Alex K.', 'Jordan T.', 'Casey L.', 'Morgan R.'].map((member, index) => (
                  <div 
                    key={member} 
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 bg-gradient-to-br from-purple-500 to-purple-600 rounded-full flex items-center justify-center">
                        <span className="text-white text-sm font-bold">{member.split(' ')[0][0]}</span>
                      </div>
